...
const Forecast = ({ displaytodaydata, setdisplaytodaydata }) => {
    const [weathericon,setweathericon]=useState(null)
  console.log("forecast display:-", displaytodaydata);
    ...
    >
      <div className="forecast-data">
        ...
        <div className="todaysresult">
          {displaytodaydata && displaytodaydata.length > 0 ? (
            displaytodaydata.map((item) => (
              <div key={item.id} >
                <p>{item.time}</p>
                <h2>{item.temp}°C</h2>
                <p>{item.desc}</p>
